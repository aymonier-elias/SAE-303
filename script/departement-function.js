...
function handleCorse(divSvg, categoryColor) {
  divSvg.innerHTML = `
...
function setupExpandableCategories(treeContainer) {
  treeContainer
    .querySelectorAll('[data-expandable="true"]')
    .forEach((expandable) => {
      expandable.addEventListener("click", function (e) {
        ...
        if (
          ...
          if (isMainCategory) {
            ...
            treeContainer
              .querySelectorAll('.category-item[data-expandable="true"]')
              .forEach((otherCategory) => {
                if (otherCategory !== this) {
                  const otherChildren =
                    otherCategory.querySelector(".category-children");
            ...
            if (parentCategory) {
              parentCategory
                .querySelectorAll('.child-item[data-expandable="true"]')
                .forEach((otherChild) => {
                  if (otherChild !== this) {
                    const otherChildren =
                      otherChild.querySelector(".child-children");
          ...
          const childItems = children.querySelectorAll(
  ...
  loadData().then((data) => {
    ...
    if (config.createChart) {
      ...
      const chartContainer = createChartContainer(
        config.chartTitle || "Comparaison avec la moyenne nationale",
      );
